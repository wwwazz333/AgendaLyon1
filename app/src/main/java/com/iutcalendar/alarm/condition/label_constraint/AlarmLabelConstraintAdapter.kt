...
class AlarmLabelConstraintAdapter(
    var context: Context?,
    var list: List<AlarmConstraintLabel>,
    var saveListener: () -> Unit
) : RecyclerView.Adapter<AlarmLabelConstraintViewHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): AlarmLabelConstraintViewHolder {
    ...
    override fun onBindViewHolder(holder: AlarmLabelConstraintViewHolder, position: Int) {
        ...
        holder.typeConstraint.setOnClickListener {
            showEditConstraintType(
                holder,
                constraintLabelAlarm
            )
        }
    ...
    private fun showEditConstraintType(
        holder: AlarmLabelConstraintViewHolder,
        constraintLabelAlarm: AlarmConstraintLabel
    ) {
        val type: Containing =
            if (holder.typeConstraint.text.toString() == Containing.MUST_NOT_CONTAIN.toString(
                    context
                )
            ) {
